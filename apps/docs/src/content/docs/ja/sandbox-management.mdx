...
Daytona SDKは、デフォルト設定、特定の言語、またはカスタムラベルを使用してSandboxを作成するメソッドをPythonとTypeScriptで提供します。
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```python # Sandboxをアーカイブする sandbox.archive() ```
  </TabItem>
  <TabItem label="TypeScript" icon="seti:typescript">
    ```typescript // Sandboxをアーカイブする await sandbox.archive(); ```
  </TabItem>
</Tabs>
