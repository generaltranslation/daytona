...
Daytona Sandboxで最初のコード行を実行しましょう。AIアシスタントでの高速開発には、[LLMsコンテキストファイル](/docs/ja/getting-started#additional-examples)をご利用ください。
...
これで、安全で分離されたコード実行にDaytonaを使用する準備が整いました。
